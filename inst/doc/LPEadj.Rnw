...
curve using the median of the gene intensites for a particular group.
...
\caption{(a) False positive rates for the LPE and adjusted LPE methods using
  ...
  threshold.  The LPE showed variable and low false positive rates.  In contrast, the adjusted LPE showed
  appropriate false positive rate for all sample sizes. (b) The adjusted LPE,
  but not the LPE, shows the theoretically expected uniform p-value
  distribution.
...
\caption{P-value histograms and boxplots of FPR, TPR, and pAUC from the
  LPE and adjusted LPE methods applied to the HGU95 latin square data set. The data were normalized using
  six different normalization methods (labeled by row). \vspace{-5mm}}
\label{fig:Power}
\end{figure}

Figure \ref{fig:Power} summarizes the results of the LPE and adjusted LPE
methods applied to the HGU95 Affymetrix spike-in data set (www.affymetrx.com). The HGU95 data is
based on a 14 x 14 Latin Square design of ``spiked-in'' 
...
(TPR, which is the proportion of transcipts correctly identified as being
